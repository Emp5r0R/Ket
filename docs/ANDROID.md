...
2. The control response is parsed fail-closed and ranked by server priority. Android accepts strict Hysteria2 UDP profiles and VLESS + REALITY profiles with Vision, a supported TLS fingerprint, and validated public-key material. Node identity, country/city, coordinates, health, capacity, CPU, memory, uptime, and observation time are range-checked before entering UI state.
