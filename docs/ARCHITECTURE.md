...
| Data-plane control | Scoped credentials, engine configuration, auth, traffic, health, kicks | Rust | Hysteria2, VLESS + REALITY, XHTTP/TLS Stealth, and Shadowsocks 2022 implemented |
| Transport engines | VLESS Reality, VLESS XHTTP/TLS, Hysteria2, Shadowsocks 2022, future maintained adapters | Maintained upstream engines | Hysteria2 2.10, Xray-core 26.3.27, and shadowsocks-rust 1.24.0 integrated server/client-side |
| Desktop client core | Node enrollment, strategy selection, tunnel lifecycle, metrics | Rust | Four implemented transports with bounded fallback |
...
| Android | `VpnService`, map-first Compose UI, shared contracts | Kotlin/Compose, Natural Earth, Android Keystore, Hysteria2, Xray, shadowsocks-rust, hev-socks5-tunnel | Four-transport parity on 64-bit API 28+; current arm64 Hysteria2/REALITY packet flow verified; Shadowsocks/XHTTP restricted-network, corrected handover, Doze/revoke, API 26, DNS leak, always-on/reboot, and owner-signing physical gates pending |
...
5. Hysteria2 submits its scoped credential to Ket's HTTP authentication backend. VLESS + REALITY and XHTTP/TLS Stealth receive the same deterministic lease-scoped UUID, which Ket atomically installs in every configured Xray inbound through the private Handler API before returning the manifest. Shadowsocks receives a deterministic 32-byte SIP022 key and a stable TCP+UDP port derived from its persisted lease resource slot; the key itself is never persisted.
6. Clients renew the lease while connected and release it on disconnect. Release, grant revocation, and the expiry reaper reject future authentication and remove or kick the session in every configured data plane. Ket reconciles persisted active leases with Xray and the Shadowsocks manager at startup before reporting ready.
7. On desktop, the unprivileged Tauri process sends validated transport requests to a loopback-only system service. The service authenticates each connection, starts the selected Hysteria, Xray, or Shadowsocks loopback SOCKS engine, then gives the shared `tun2proxy` bridge full-route and virtual-DNS ownership while bypassing every pre-resolved server IP.
...
- Desktop Hysteria, Reality, XHTTP Stealth, and Shadowsocks require the supervised bridge's virtual-DNS mode; the bridge captures IPv4 and IPv6, while each engine connects to pre-resolved server or CDN IPs outside those routes.
...
- Android native inputs are reproducible: Hysteria, Xray, and Shadowsocks executables plus the complete hev source archive are version- and checksum-pinned, and CI verifies the expected payload matrix.
...
`Ikev2`, `OpenVpnStunnel`, and `WireGuard` remain discovery identifiers rather than executable adapters. Future implementations must use maintained engines while preserving lease-scoped credentials, live revocation, per-session accounting where the engine supports it, and Android/Desktop parity.
