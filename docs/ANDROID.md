...
3. The validated manifest is ranked by server priority. Android accepts at most 32 profiles with bounded unique identifiers and credentials: strict Hysteria2 UDP settings; VLESS + REALITY with Vision and validated public-key material; XHTTP `packet-up` with certificate-verified TLS and a bounded path; Shadowsocks 2022 with the exact SIP022 method and 32-byte key; or WireGuard TLS with exact routes, address allocation, WebSocket carrier, and three 32-byte keys. Node identity, country/city, coordinates, health, capacity, CPU, memory, uptime, and observation time are range-checked before entering UI state.
4. A successful manual enrollment atomically persists the normalized control endpoint, access grant, and complete current session manifest in an authenticated encrypted record. An app launch is explicitly flagged; an unflagged system launch restores the current lease for always-on VPN. Ket renews a saved session before use, replaces it only after confirmed authorization loss, and retains it optimistically when a restricted network blocks the control endpoint.
5. The service attempts each supported transport in rank order. An unavailable upstream payload, failed WireGuard TLS, XHTTP/TLS, REALITY, or Shadowsocks path check, or other retryable startup failure falls through to the next ranked transport without weakening any profile. Successful latency and failures feed a bounded exponential-cooldown history.
...
The Compose surface uses the same node contract as desktop. It renders the bundled Natural Earth 1:110m land topology with the server's latitude/longitude, a restrained connection pulse, health-aware capacity, local transfer counters, transport/handshake details, and live server CPU, memory, uptime, and session metrics. The layout scrolls under constrained height while the primary connection control remains outside the scrolling content. The Natural Earth-derived asset and its redistribution notice are tracked with the Android source.

## Native supply chain

`packaging/prepare-android-engines.sh` downloads checksum-pinned Hysteria 2.10 executables for `armeabi-v7a`, `arm64-v8a`, `x86`, and `x86_64`, Xray-core 26.3.27 and shadowsocks-rust 1.24.0 for `arm64-v8a` and `x86_64`, and wstunnel 10.6.2 for `arm64-v8a`. It also verifies and expands the complete hev-socks5-tunnel 2.14.0 source release. NDK r27d compiles hev and Ket's small API-26-compatible FD receiver for every ABI. The official Shadowsocks and wstunnel binaries target Android API 28; wstunnel is currently arm64-only. Unsupported devices retain ranked fallback rather than weakening a profile.

Generated third-party files live under `app/build/generated/ket-engines` and are not tracked. The APK validator requires Hysteria, hev, and the Ket JNI shim for every ABI, Xray plus Shadowsocks for both supported 64-bit ABIs, and wstunnel for arm64 before CI uploads the artifact.
...
Repeat the corrected switch test on the API 36 device, exercise WireGuard TLS, XHTTP/TLS, and Shadowsocks 2022 on the restricted network, and run the Hysteria-only compatibility matrix on a physical API 26 device. The implemented Doze-exit renewal, graceful VPN-permission revoke, process/reboot restoration, always-on/lockdown lifecycle, explicit connected-state DNS leak behavior, and owner-signed release installation remain physical-device release gates. Release signing material is intentionally not stored in this repository.
