...
| Data-plane control | Scoped credentials, engine configuration, auth, traffic, health, kicks | Rust | Hysteria2, VLESS + REALITY, and XHTTP/TLS Stealth implemented |
| Transport engines | VLESS Reality, VLESS XHTTP/TLS, Hysteria2, future maintained adapters | Maintained upstream engines | Hysteria2 2.10 and Xray-core 26.3.27 integrated server/client-side |
| Desktop client core | Node enrollment, strategy selection, tunnel lifecycle, metrics | Rust | Hysteria2, VLESS + REALITY, and XHTTP/TLS Stealth implemented |
...
| Android | `VpnService`, map-first Compose UI, shared contracts | Kotlin/Compose, Natural Earth, Android Keystore, Hysteria2, Xray, hev-socks5-tunnel | Three-transport parity implemented on 64-bit; current arm64 Hysteria2/REALITY packet flow verified; XHTTP restricted-network, corrected handover, Doze/revoke, API 26, DNS leak, always-on/reboot, and owner-signing physical gates pending |
...
- Desktop Hysteria, Reality, and XHTTP Stealth require the supervised bridge's virtual-DNS mode; the bridge captures IPv4 and IPv6, while each engine connects to pre-resolved server or CDN IPs outside those routes.
...
`Ikev2`, `OpenVpnStunnel`, `Shadowsocks2022`, and `WireGuard` remain discovery identifiers rather than executable adapters. The pinned Xray 26.3.27 engine warns that its Shadowsocks implementation is deprecated and may be removed, so Ket will not bind a new production Shadowsocks transport to that lifecycle. Future implementations must use maintained engines while preserving lease-scoped credentials, live revocation, per-session accounting, and Android/Desktop parity.
