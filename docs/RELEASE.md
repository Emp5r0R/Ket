...
The maintained server and Linux/Windows data planes are Hysteria2, Xray-core VLESS + REALITY, Xray-core VLESS + XHTTP/TLS Stealth, shadowsocks-rust Shadowsocks 2022, WireGuard over wstunnel WebSocket/TLS, and OpenVPN over stunnel TLS. SOCKS-backed desktop transports share a locally tested full-route bridge; OpenVPN owns a separately supervised native TUN. Android Hysteria2 and REALITY packet flow, startup fallback, engine-exit recovery, bidirectional Wi-Fi/cellular recovery, cancellation, and disconnect have been exercised on a physical current arm64 device; WireGuard TLS, XHTTP, and Shadowsocks currently have strict local parsing, engine, and package validation only, while OpenVPN is intentionally skipped pending a native Android bridge. Fail-closed handover, restricted-network traffic, Doze-exit renewal, graceful permission-revoke, always-on/reboot, API 26, connected-state DNS-leak, lockdown, and owner-signed release tests remain. WireGuard and OpenVPN server lifecycles must be exercised on a capable Linux deployment host. IKEv2 and XOR remain contract-level extension points rather than implemented adapters.
