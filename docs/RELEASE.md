...
The maintained server and client data planes are Hysteria2, Xray-core VLESS + REALITY, Xray-core VLESS + XHTTP/TLS Stealth, and shadowsocks-rust Shadowsocks 2022. All desktop transports share a locally tested full-route bridge that enforces virtual DNS, IPv4/IPv6 capture, endpoint-IP bypass, paired process supervision, and cleanup; REALITY also has a full-route Docker integration test. Android Hysteria2 and REALITY packet flow, startup fallback, engine-exit recovery, bidirectional Wi-Fi/cellular recovery, cancellation, and disconnect have been exercised on a physical current arm64 device; XHTTP and Shadowsocks currently have strict local parsing, engine, and package validation only. Fail-closed handover, XHTTP/Shadowsocks restricted-network traffic, Doze-exit renewal, graceful permission-revoke, and always-on/reboot corrections need physical API 36 repeats; Android API 26, connected-state DNS-leak, lockdown, and owner-signed release testing also remain. Always-on support is enabled only after implementing a Keystore-sealed durable enrollment/session record and boot-safe session resolution. IKEv2, OpenVPN/stunnel, WireGuard, and XOR remain contract-level extension points rather than implemented adapters.
