...
Ket Android consumes the versioned control API directly and implements platform-owned Hysteria2, VLESS + REALITY, XHTTP/TLS Stealth, and Shadowsocks 2022 adapters. It does not run the desktop broker or attempt desktop-style privileged route changes.
...
3. The validated manifest is ranked by server priority. Android accepts at most 32 profiles with bounded unique identifiers and credentials: strict Hysteria2 UDP settings, VLESS + REALITY with Vision and validated public-key material, XHTTP `packet-up` with certificate-verified TLS and a bounded path, or Shadowsocks 2022 with the exact SIP022 method, a 32-byte key, and no extra secrets. Node identity, country/city, coordinates, health, capacity, CPU, memory, uptime, and observation time are range-checked before entering UI state.
...
5. The service attempts each supported transport in rank order. An unavailable 64-bit Xray/Shadowsocks payload, failed XHTTP/TLS, REALITY, or Shadowsocks path check, or other retryable startup failure falls through to the next ranked transport without weakening any profile. Successful latency and failures feed a bounded exponential-cooldown history.
...
7. Before installing a VPN route, Ket resolves each advertised data-plane endpoint and pins the selected engine to a numeric address. Xray starts with a loopback SOCKS inbound, validates its generated REALITY or XHTTP configuration, and proves the selected path with a certificate-verified request through SOCKS. Shadowsocks starts `libsslocal.so` with an ephemeral strict TCP+UDP configuration and completes the same certificate-verified SOCKS test. The pinned endpoint or CDN addresses are excluded from the VPN to prevent recursion during startup and ranked fallback.
...
`packaging/prepare-android-engines.sh` downloads checksum-pinned Hysteria 2.10 executables for `armeabi-v7a`, `arm64-v8a`, `x86`, and `x86_64`, plus Xray-core 26.3.27 and shadowsocks-rust 1.24.0 for `arm64-v8a` and `x86_64`. It also verifies and expands the complete hev-socks5-tunnel 2.14.0 source release. NDK r27d compiles hev and Ket's small API-26-compatible FD receiver for every ABI. The official Shadowsocks binaries target Android API 28 and are 64-bit only; 32-bit and API 26-27 devices retain Hysteria2 fallback. REALITY and XHTTP/TLS are also 64-bit Android transports.

Generated third-party files live under `app/build/generated/ket-engines` and are not tracked. The APK validator requires Hysteria, hev, and the Ket JNI shim for every ABI, and Xray plus Shadowsocks for both supported 64-bit ABIs, before CI uploads the artifact.
