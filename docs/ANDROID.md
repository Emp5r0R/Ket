...
This policy covers an established or recovering Ket tunnel. Android's system lockdown option is what blocks traffic before a VPN connects, but Ket cannot yet restore its memory-only session after a system boot. The manifest therefore opts out of [always-on VPN](https://developer.android.com/develop/connectivity/vpn#always-on) instead of exposing a nonfunctional control. Durable device enrollment and boot-safe reconnection must be implemented before Ket enables always-on and lockdown support.
...
Repeat the corrected switch test on the API 36 device and the complete matrix on a physical API 26 device. The implemented Doze-exit renewal and graceful VPN-permission revoke paths, explicit connected-state DNS leak behavior, and owner-signed release installation remain physical-device release gates. Durable device enrollment and boot-safe reconnection remain implementation gates for always-on/lockdown support. Release signing material is intentionally not stored in this repository.
