...
| Server data planes | Compose transport overlays | Hysteria2, REALITY, public Shadowsocks TCP+UDP, and public OpenVPN/stunnel nested TLS live-tested on Oracle ARM64; XHTTP/TLS traffic/lifecycle locally verified; WireGuard TLS deployment verified with restricted-network traffic pending |
...
The maintained server and Linux/Windows data planes are Hysteria2, Xray-core VLESS + REALITY, Xray-core VLESS + XHTTP/TLS Stealth, shadowsocks-rust Shadowsocks 2022, WireGuard over wstunnel WebSocket/TLS, and OpenVPN over stunnel TLS. SOCKS-backed desktop transports share a locally tested full-route bridge; OpenVPN owns a separately supervised native TUN. Android implements all six through platform-owned routing, including a private OpenVPN management/TUN bridge and certificate-pinned stunnel-compatible TLS carrier. Android Hysteria2 and REALITY packet flow, startup fallback, engine-exit recovery, bidirectional Wi-Fi/cellular recovery, cancellation, and disconnect have been exercised on a physical current arm64 device. On 2026-07-22, an owner-signed APK additionally passed restricted-network VLESS + REALITY with bridge-local mapped DNS, an end-to-end HTTPS 204 gate, Oracle public egress confirmation, nonzero transfer, automatic client cleanup, restored device egress, and grant revocation. XHTTP has local certificate-verified TLS traffic, scoped UUID, counters, release, and revocation proof through real Xray engines. Shadowsocks TCP traffic, key enforcement, counters, and revocation are proven locally through real upstream engines; the Oracle deployment additionally passed authenticated public TCP egress and UDP DNS forwarding, followed by listener removal on grant revocation. WireGuard enrollment and peer revocation, plus the OpenVPN agent/stunnel stack, are deployed on Oracle ARM64. OpenVPN has local lifecycle proof and a public certificate-pinned nested-TLS handshake with scoped authentication and nonzero telemetry after strict trust-chain repair. Restricted-network Android gates remain for Hysteria2, XHTTP, Shadowsocks, WireGuard, and OpenVPN, along with TUN-backed public OpenVPN packet flow, fail-closed handover, Doze-exit renewal, graceful permission-revoke, always-on/reboot, API 26, connected-state DNS-leak, lockdown, CDN routing, sustained transfer, and owner-signed Windows installation. IKEv2 and XOR remain contract-level extension points rather than implemented adapters.
