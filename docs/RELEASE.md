...
| Server data planes | Compose transport overlays | Hysteria2 and REALITY live-tested on Oracle ARM64; XHTTP/TLS Stealth, Shadowsocks, and WireGuard TLS locally validated with deployment gates pending |
...
| Android debug APK | `./packaging/build-android.sh` | Five-transport parser/engine parity on arm64 API 28+; Hysteria2/REALITY packet flow and recovery exercised on current hardware; WireGuard TLS/XHTTP/Shadowsocks restricted-network tests pending |
...
The maintained server and client data planes are Hysteria2, Xray-core VLESS + REALITY, Xray-core VLESS + XHTTP/TLS Stealth, shadowsocks-rust Shadowsocks 2022, and WireGuard over wstunnel WebSocket/TLS. All desktop transports share a locally tested full-route bridge that enforces virtual DNS, IPv4/IPv6 capture, endpoint-IP bypass, multi-process supervision, and cleanup; REALITY also has a full-route Docker integration test. Android Hysteria2 and REALITY packet flow, startup fallback, engine-exit recovery, bidirectional Wi-Fi/cellular recovery, cancellation, and disconnect have been exercised on a physical current arm64 device; WireGuard TLS, XHTTP, and Shadowsocks currently have strict local parsing, engine, and package validation only. Fail-closed handover, restricted-network traffic for those three transports, Doze-exit renewal, graceful permission-revoke, and always-on/reboot corrections need physical API 36 repeats; Android API 26, connected-state DNS-leak, lockdown, and owner-signed release testing also remain. WireGuard's kernel peer lifecycle must also be exercised on a capable Linux deployment host. IKEv2, OpenVPN/stunnel, and XOR remain contract-level extension points rather than implemented adapters.
